...
namespace EdFi.AnalyticsMiddleTier.Tests.Dimensions.StudentSchoolDimTestGroup
{
    ...
    public abstract class When_querying_the_StudentSchoolDim_view : When_querying_a_view_postgres
    {
        ...
        {
            (bool success, string errorMessage) testResult = DataStandard.RunTestCase<TableColumns>($"{TestCasesFolder}.{DataStandard.TestDataFolderName}.0001_StudentSchoolDim_should_match_column_dictionary.xml");
            testResult.success.ShouldBe(true, testResult.errorMessage);
        }

        [SetUpFixture]
        public class SetupStudentSchoolDimTestCase
                : When_querying_the_StudentSchoolDim_view
        {
            [OneTimeSetUp]
            public void PrepareDatabase() => PrepareTestData<StudentSchoolDim>(TestCasesFolder, TestCasesDataFileName);
        }
